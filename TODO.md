...
**An unexplained observation** (PSRP-15). An intermittent failure seen twice
while cycling transports in a tight loop, not reproduced since. Kept open
because it was observed and not explained.
...
| PSRP-15 | Transport | 2.1 | Intermittent failure when cycling transports in a tight loop | Observed twice while stress-testing create-transport / open / run / close / free in a loop: the run failed at cycle 41 and, after a first round of fixes, at cycle 48. Not reproduced since, across 200 further cycles, and never seen in ordinary use. WinRM quotas on the test machine are unlimited, so it is not a server limit. Recorded rather than dismissed because it was seen and not explained; the fixes made in the same session were justified on their own terms and are not claimed to have cured it. Anyone reproducing it should capture psrp_transport_last_error at the failing step, which the throwaway harness now prints. | open |
