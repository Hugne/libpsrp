...
| PSRP-17 | Metadata | 2.2.2.14 | GET_COMMAND_METADATA always sends a Null Namespace | The builder does not expose the Namespace property, so every request sends Null, which 2.2.2.14 defines as a list holding one empty string. Measured against Windows PowerShell: "*" returns 1166 commands and an exact name returns exactly that command, but "Get-Ch*" and "*ChildItem*" return none, which looks like the server filtering on that empty namespace. The encoding is faithful to the spec and both useful shapes work; what is missing is a way for a caller to ask for a different namespace. | open |
