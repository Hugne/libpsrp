                                             ...
                                             size_t len)
{
    ...
    /* A decrypt that produced the wrong number of bytes is a cryptographic
     * failure, not a structural one, and the OpenSSL backend reaches this
     * same line for a bad ciphertext because implicit rejection hides the
     * padding error from it. Same input, same answer on both. */
    if (produced != PSRP_SESSION_KEY_BYTES) return PSRP_ERR_CRYPTO;
